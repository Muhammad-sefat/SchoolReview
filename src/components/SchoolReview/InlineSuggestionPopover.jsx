...
const GreenCheckIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none" className="shrink-0 mt-0.5">
    <path
      fillRule="evenodd"
      clipRule="evenodd"
...
const InlineSuggestionPopover = ({
  ...
  return (
    <div className="w-full max-w-[540px] bg-white rounded-[20px] border border-border/80 shadow-lg p-5 space-y-4 font-urbanist animate-in fade-in zoom-in-95 duration-200 mt-3 relative z-30">
      {/* Title */}
      <div className="flex items-center gap-2.5">
        <TopTitleCheckCircleIcon />
        <h3 className="text-[24px] font-semibold text-[#080808]">Suggestion</h3>
      </div>

      {/* Warning Notice Banner */}
      <div className="bg-[#EBF5FF] border border-[#BEE0FF] px-4 py-3 rounded-xl flex items-center gap-2.5">
        <WarningNoticeIcon />
      ...
      <div className="flex items-center gap-2 pt-1">
        <button
          ...
          className={`px-4 py-1.5 rounded-full text-[14px] transition-all cursor-pointer ${
            activeTab === "use"
              ? "bg-[#038AF9] text-white font-medium shadow-xs"
              : "border border-border/80 text-secondary font-normal bg-background hover:bg-muted/40"
          }`}
        >
          Use suggestion
        </button>
      ...
      <div className="space-y-3 pt-1">
        {/* Original Text Card (Red Close Icon) */}
        <div className="p-4 rounded-xl border border-border/80 bg-white flex items-start gap-3">
          <RedCloseIcon />
        ...
        {/* Suggested Text Card (Green Check Icon) */}
        <div className="p-4 rounded-xl border border-border/80 bg-white flex items-start gap-3">
          <GreenCheckIcon />
          <span className="text-[16px] font-normal text-textPrimary leading-relaxed">
            {suggestedText}
      ...
      <div className="pt-2">
        <button
          type="button"
          onClick={() => {
            if (activeTab === "use" && onUseSuggestion) {
...
export default InlineSuggestionPopover
