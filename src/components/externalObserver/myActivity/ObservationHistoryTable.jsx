...
const VideoIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
    <g clipPath="url(#clip0_10748_248864_history)">
    ...
    <defs>
      <clipPath id="clip0_10748_248864_history">
...
const ObservationHistoryTable = () => {
  ...
  const handleViewObservation = (row) => {
  ...
  return (
    <div className="bg-white border border-gray-200/80 rounded-3xl p-6 sm:p-8 shadow-2xs space-y-6 font-urbanist">
      ...
      <div className="space-y-1.5 pb-4 border-b border-dashed border-gray-200">
        <Title24 className="text-textBlack font-semibold">Observation History</Title24>
        <p className="text-base font-normal text-textBlack leading-relaxed">
          External observers can access their own submitted observations and activity history. Full teacher insight reports remain private unless shared directly by the teacher.
        </p>
      </div>
      ...
      <Table containerClassName="overflow-x-auto border border-gray-100 rounded-2xl bg-white shadow-2xs w-full max-w-full" className="w-full min-w-[700px]">
        <TableHeader>
          <TableRow className="border-b border-gray-100 hover:bg-transparent bg-gray-50/40">
            ...
            <TableHead className="text-[16px] font-medium text-[#5A5A5A] py-4 whitespace-nowrap">Teaching Material</TableHead>
            <TableHead className="text-[16px] font-medium text-[#5A5A5A] text-center pr-6 py-4 whitespace-nowrap">Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {historyData.length > 0 ? (
            historyData.map((row) => (
              <TableRow key={row.id} className="border-b border-gray-50/80 hover:bg-gray-50/50 transition-colors">
                ...
                <TableCell className="text-center pr-6 py-5 whitespace-nowrap">
                  <div className="flex items-center justify-center gap-3">
                    <button
                      type="button"
                      onClick={() => handleViewObservation(row)}
                      className="p-1.5 rounded-full bg-gray-100 text-[#5A5A5A] hover:text-[#038AF9] transition-colors cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-full bg-gray-100 text-[#5A5A5A] hover:text-red-500 transition-colors cursor-pointer"
                      title="Delete Entry"
                    >
...
export default ObservationHistoryTable
