...
const StudentFeedbackIcon = ({ className = "w-5 h-5" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" className={className}>
    <g clipPath="url(#clip0_13198_72478_obs_sidebar)">
      <mask id="mask0_13198_72478_obs_sidebar" style={{ maskType: "luminance" }} maskUnits="userSpaceOnUse" x="0" y="0" width="24" height="24">
        <path d="M23.5 0.5V23.5H0.5V0.5H23.5Z" fill="white" stroke="white" />
      </mask>
      <g mask="url(#mask0_13198_72478_obs_sidebar)">
    ...
    <defs>
      <clipPath id="clip0_13198_72478_obs_sidebar">
...
const ExternalObserverSidebar = ({ open, setOpen, collapsed, setCollapsed }) => {
  ...
  const isMyWorkActive = location.pathname.includes("/external-observer/my-work")
  ...
  useEffect(() => {
    if (!location.pathname.includes("/external-observer/my-work")) {
      setMyWorkOpen(false)
    }
  ...
  const isPathActive = (itemPath) => {
    if (itemPath === "/external-observer" || itemPath === "/external-observer/teaching-insights") {
      return (
        location.pathname === "/external-observer" ||
        location.pathname === "/external-observer/" ||
        location.pathname === "/external-observer/teaching-insights"
      )
    }
      ...
      >
        ...
        <div className="shrink-0">
          ...
          <nav className={`px-3.5 py-4 space-y-3 font-urbanist `}>
            {/* 1. Teaching Insights */}
            <NavLink
              to="/external-observer/teaching-insights"
              className={`flex items-center px-3.5 py-3 rounded-xl text-base transition-all duration-300 ease-in-out group ${isPathActive("/external-observer/teaching-insights")
            ...
            >
              <div className="shrink-0 flex items-center justify-center">
                <TeachingInsightsIcon
                  className={`w-[22px] h-[22px] transition-colors ${isPathActive("/external-observer/teaching-insights")
                      ? "text-[#080808]"
            ...
            {/* 2. Student Feedback */}
            <NavLink
              to="/external-observer/student-feedback"
              className={`flex items-center px-3.5 py-3 rounded-xl text-base transition-all duration-300 ease-in-out group ${location.pathname.includes("/external-observer/student-feedback")
                  ? "bg-[#FDFDFD] text-[#080808] border border-[#EAEAEA] shadow-2xs font-semibold"
                  : "text-[#5A5A5A] font-medium hover:bg-gray-50"
                } ${collapsed ? "justify-center px-0" : ""}`}
            ...
            >
              <div className="shrink-0 flex items-center justify-center relative">
                <StudentFeedbackIcon
                  className={`w-[22px] h-[22px] transition-colors ${location.pathname.includes("/external-observer/student-feedback")
                      ? "text-[#080808]"
                      : "text-[#5A5A5A] group-hover:text-[#1F1F21]"
                    }`}
                />
            ...
            <div className="space-y-1 relative group">
              ...
              {!collapsed && myWorkOpen && (
                <div className="pl-9 pr-2 space-y-6 py-1.5 animate-fadeIn">
                  <NavLink
                    to="/external-observer/my-work/activity-task"
                    className={({ isActive }) =>
