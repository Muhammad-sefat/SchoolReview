import React, { useState } from "react"
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog"
import { DatePicker } from "@/components/ui/date-picker"

const CalendarIconSVG = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
    <path fillRule="evenodd" clipRule="evenodd" d="M8.1 1.25C8.63848 1.25 9.075 1.68754 9.075 2.22727V3.21284C9.6529 3.20453 10.2784 3.20453 10.9537 3.20454H13.0463C13.7216 3.20453 14.3471 3.20453 14.925 3.21284V2.22727C14.925 1.68754 15.3615 1.25 15.9 1.25C16.4385 1.25 16.875 1.68754 16.875 2.22727V3.29947C17.0513 3.31556 17.221 3.33437 17.3843 3.35638C18.5544 3.51407 19.5397 3.85141 20.3221 4.63573C21.1046 5.42004 21.4412 6.40758 21.5985 7.58045C21.75 8.71014 21.75 10.1465 21.75 11.9285V14.026C21.75 15.808 21.75 17.2444 21.5985 18.3741C21.4412 19.547 21.1046 20.5345 20.3221 21.3188C19.5397 22.1031 18.5544 22.4405 17.3843 22.5982C16.2572 22.75 14.8242 22.75 13.0463 22.75H10.9537C9.17581 22.75 7.74279 22.75 6.61573 22.5982C5.44558 22.4405 4.46035 22.1031 3.67786 21.3188C2.89537 20.5345 2.5588 19.547 2.40148 18.3741C2.24995 17.2444 2.24998 15.8081 2.25 14.0261V11.9285C2.24998 10.1465 2.24995 8.71012 2.40148 7.58045C2.5588 6.40758 2.89537 5.42004 3.67786 4.63573C4.46035 3.85141 5.44558 3.51407 6.61573 3.35638C6.77902 3.33437 6.94874 3.31556 7.125 3.29947V2.22727C7.125 1.68754 7.56152 1.25 8.1 1.25ZM4.21386 9.75C4.20045 10.3958 4.20001 11.1378 4.20001 12V13.9545C4.20001 15.8249 4.20208 17.1294 4.3341 18.1137C4.46235 19.0698 4.69693 19.5761 5.05673 19.9367C5.41652 20.2974 5.92166 20.5325 6.87557 20.6611C7.85752 20.7934 9.15897 20.7955 11.025 20.7955H12.975C14.8411 20.7955 16.1425 20.7934 17.1245 20.6611C18.0784 20.5325 18.5835 20.2974 18.9433 19.9367C19.3031 19.5761 19.5377 19.0698 19.6659 18.1137C19.7979 17.1294 19.8 15.8249 19.8 13.9545V12C19.8 11.1378 19.7996 10.3958 19.7862 9.75H4.21386Z" fill="#BFBFBF"/>
    <path fillRule="evenodd" clipRule="evenodd" d="M7 13.2734C7 12.7212 7.44772 12.2734 8 12.2734H8.00897C8.56126 12.2734 9.00897 12.7212 9.00897 13.2734C9.00897 13.8257 8.56126 14.2734 8.00897 14.2734H8C7.44772 14.2734 7 13.8257 7 13.2734ZM10.9955 13.2734C10.9955 12.7212 11.4432 12.2734 11.9955 12.2734H12.0045C12.5568 12.2734 13.0045 12.7212 13.0045 13.2734C13.0045 13.8257 12.5568 14.2734 12.0045 14.2734H11.9955C11.4432 14.2734 10.9955 13.8257 10.9955 13.2734ZM14.991 13.2734C14.991 12.7212 15.4387 12.2734 15.991 12.2734H16C16.5523 12.2734 17 12.7212 17 13.2734C17 13.8257 16.5523 14.2734 16 14.2734H15.991C15.4387 14.2734 14.991 13.8257 14.991 13.2734ZM7 17.2734C7 16.7212 7.44772 16.2734 8 16.2734H8.00897C8.56126 16.2734 9.00897 16.7212 9.00897 17.2734C9.00897 17.8257 8.56126 18.2734 8.00897 18.2734H8C7.44772 18.2734 7 17.8257 7 17.2734ZM10.9955 17.2734C10.9955 16.7212 11.4432 16.2734 11.9955 16.2734H12.0045C12.5568 16.2734 13.0045 16.7212 13.0045 17.2734C13.0045 17.8257 12.5568 18.2734 12.0045 18.2734H11.9955C11.4432 18.2734 10.9955 17.8257 10.9955 17.2734Z" fill="#BFBFBF"/>
  </svg>
)

const RemindMeLaterModal = ({ isOpen, onClose, onSetReminder }) => {
  const [selectedPill, setSelectedPill] = useState(null) // default nothing selected
  const [reminderDate, setReminderDate] = useState(new Date(2026, 7, 12)) // 12 Aug 2026

  if (!isOpen) return null

  const handlePillClick = (pillKey) => {
    setSelectedPill(pillKey)
  }

  const handleSetReminder = () => {
    if (onSetReminder) {
      onSetReminder({ pill: selectedPill, date: reminderDate })
    }
    onClose()
  }

  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[540px] w-[92vw] rounded-[32px] p-6 sm:p-8 space-y-6 bg-white border border-gray-100 shadow-2xl font-urbanist">
        {/* Header */}
        <DialogHeader className="p-0 space-y-0 text-left flex flex-row items-center justify-between border-b border-dashed border-gray-200 pb-4">
          <DialogTitle asChild>
            <h2 className="text-[24px] sm:text-[28px] font-semibold text-[#080808]">
              When Should We Remind You?
            </h2>
          </DialogTitle>
        </DialogHeader>

        {/* Pill Selection Buttons */}
        <div className="flex items-center gap-3 flex-wrap pt-2">
          <button
            type="button"
            onClick={() => handlePillClick("1week")}
            className={`px-5 py-2.5 rounded-full text-base font-medium transition-all cursor-pointer ${selectedPill === "1week"
              ? "bg-[#038AF9] text-white shadow-xs"
              : "border border-gray-200 bg-white text-secondary hover:bg-gray-50"
              }`}
          >
            In 1 Week
          </button>

          <button
            type="button"
            onClick={() => handlePillClick("2weeks")}
            className={`px-5 py-2.5 rounded-full text-base font-medium transition-all cursor-pointer ${selectedPill === "2weeks"
              ? "bg-[#038AF9] text-white shadow-xs"
              : "border border-gray-200 bg-white text-secondary hover:bg-gray-50"
              }`}
          >
            In 2 Weeks
          </button>

          <button
            type="button"
            onClick={() => handlePillClick("chooseDate")}
            className={`px-5 py-2.5 rounded-full text-base font-medium transition-all cursor-pointer ${selectedPill === "chooseDate"
              ? "bg-[#038AF9] text-white shadow-xs"
              : "border border-gray-200 bg-white text-secondary hover:bg-gray-50"
              }`}
          >
            Choose Date
          </button>
        </div>

        {/* Bottom Date Section (Shown when Choose Date is clicked or active) */}
        {selectedPill === "chooseDate" && (
          <div className="space-y-2 pt-2 animate-fadeIn">
            <label className="text-[18px] font-medium text-textPrimary block">
              Select a date
            </label>
            <div className="w-full">
              <DatePicker
                value={reminderDate}
                onChange={setReminderDate}
                placeholder="Select date"
                customIcon={<CalendarIconSVG />}
                iconPosition="right"
                formatPattern="dd MMM yyyy"
                className="h-14 rounded-2xl border-gray-200 px-4 text-base text-[#080808] w-full"
              />
            </div>
          </div>
        )}

        {/* Modal Buttons */}
        <div className="grid grid-cols-2 gap-4 pt-4">
          <button
            type="button"
            onClick={onClose}
            style={{ borderRadius: "16px", background: "#FFFFFF" }}
            className="w-full py-3 px-6 text-base font-medium text-[#080808] border border-gray-200 hover:bg-gray-50 transition-colors cursor-pointer"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={handleSetReminder}
            className="w-full py-3 px-6 text-base font-medium text-white bg-[#038AF9] hover:bg-[#0270ce] rounded-2xl transition-colors cursor-pointer shadow-xs"
          >
            Set Reminder
          </button>
        </div>
      </DialogContent>
    </Dialog>
  )
}

export default RemindMeLaterModal
