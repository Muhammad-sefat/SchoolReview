...
const StudentFeedbackIcon = ({ className = "w-5 h-5" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" className={className}>
    ...
    <defs>
      <clipPath id="clip0_13198_72478_ext_sidebar">
...
const ExternalSidebar = ({ open, setOpen, collapsed, setCollapsed }) => {
  const location = useLocation()
  const navigate = useNavigate()

  const isMyWorkActive = location.pathname.includes("/external-teacher/my-work")
  const [myWorkOpen, setMyWorkOpen] = useState(isMyWorkActive)

  const isSettingsActive = location.pathname.includes("/external-teacher/setting")
  ...
  useEffect(() => {
    if (!location.pathname.includes("/external-teacher/my-work")) {
      setMyWorkOpen(false)
    }
    if (!location.pathname.includes("/external-teacher/setting")) {
      setSettingsOpen(false)
    }
  }, [location.pathname])

  const isPathActive = (itemPath) => {
    if (itemPath === "/external-teacher" || itemPath === "/external-teacher/teaching-insights") {
      return (
        location.pathname === "/external-teacher" ||
        location.pathname === "/external-teacher/" ||
        location.pathname === "/external-teacher/teaching-insights"
      )
    }
        ...
        <div className="shrink-0">
          ...
          <nav className={`px-3.5 py-4 space-y-3 font-urbanist `}>
            ...
            <NavLink
              ...
              className={`flex items-center px-3.5 py-3 rounded-xl text-base transition-all duration-300 ease-in-out group ${isPathActive("/external-teacher/teaching-insights")
            ...
            >
              <div className="shrink-0 flex items-center justify-center">
                <TeachingInsightsIcon
                  className={`w-[22px] h-[22px] transition-colors ${isPathActive("/external-teacher/teaching-insights")
                      ? "text-[#080808]"
              ...
              >
                <span className="truncate">Teaching Insights</span>
              </div>
            </NavLink>
            ...
            <NavLink
              ...
              className={`flex items-center px-3.5 py-3 rounded-xl text-base transition-all duration-300 ease-in-out group ${location.pathname.includes("/external-teacher/student-feedback")
                  ? "bg-[#FDFDFD] text-[#080808] border border-[#EAEAEA] shadow-2xs font-semibold"
                  : "text-[#5A5A5A] font-medium hover:bg-gray-50"
                } ${collapsed ? "justify-center px-0" : ""}`}
            ...
            >
              <div className="shrink-0 flex items-center justify-center relative">
                <StudentFeedbackIcon
                  className={`w-[22px] h-[22px] transition-colors ${location.pathname.includes("/external-teacher/student-feedback")
                      ? "text-[#080808]"
                      : "text-[#5A5A5A] group-hover:text-[#1F1F21]"
                    }`}
                />
            ...
            <div className="space-y-1 relative group">
              <button
                ...
                className={`w-full flex items-center transition-all duration-300 ease-in-out group cursor-pointer ${myWorkOpen && isMyWorkActive
