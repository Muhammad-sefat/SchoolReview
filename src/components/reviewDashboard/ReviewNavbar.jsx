...
import NotificationsPopover from "@/components/leaderDashboard/common/NotificationsPopover"

const ReviewNavbar = ({ open, setOpen, user }) => {
  const [showNotifications, setShowNotifications] = useState(false)
  const [unreadCount, setUnreadCount] = useState(1)

  const userName = user?.name || "Alex Wilkerson"
    ...
    <header className="w-full h-[68px] min-h-[68px] bg-white border-b border-gray-100 px-4 md:px-8 flex items-center justify-between gap-4 sticky top-0 z-30 font-urbanist shrink-0">
      ...
      <div className="flex-1 max-w-sm">
        <div className="relative flex items-center">
          ...
          <input
            ...
            className="w-full h-10 pl-10 pr-4 bg-white border border-gray-200/90 rounded-[10px] text-xs sm:text-sm text-[#080808] placeholder:text-[#5A5A5A] focus:outline-none focus:border-[#038AF9] focus:ring-1 focus:ring-[#038AF9] transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 md:gap-6 relative">
        ...
        <div className="flex items-center gap-2.5 cursor-pointer p-1 rounded-xl hover:bg-gray-50 transition-colors">
          ...
          <span className="text-base font-medium text-[#080808] hidden sm:inline-block">
            {userName}
          </span>
          <ChevronDown className="w-4 h-4 text-[#5A5A5A] stroke-[1.75]" />
        </div>
        ...
        {/* Standalone Notifications Popover */}
        {showNotifications && (
