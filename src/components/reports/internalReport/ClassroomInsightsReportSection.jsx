...
import CategoryInsightsTable from "@/components/leaderDashboard/modal/scatterPlotModalTeacher/CategoryInsightsTable"
import TeachingQualityRadarChart from "@/components/leaderDashboard/modal/scatterPlotModalTeacher/TeachingQualityRadarChart"
...
const ClassroomInsightsReportSection = () => {
  return (
    <div className="w-full space-y-6 font-urbanist">
      ...
      {/* Render All 5 Categories One By One, Each Wrapped in Outer Container Class */}
      <div className="space-y-8">
        {CATEGORIES.map((category) => {
          const radarData = RADAR_DATA_MAP[category] || null
          const legends = category === "Professional Practice" ? TWO_LEGENDS : THREE_LEGENDS
          const rows = CATEGORY_INSIGHTS_DATA_MAP[category] || []

          return (
            <div
              key={category}
              className="bg-[#F6F6F6] p-2 rounded-4xl border border-gray-200/60 font-urbanist w-full max-w-full"
            >
              <div className="bg-white rounded-3xl p-6 sm:p-8 space-y-6">
                {/* Render title ONLY if there is no radar chart (since TeachingQualityRadarChart renders its own title) */}
                {!radarData && (
                ...
                {/* Radar Chart if exists for category */}
                {radarData && (
                ...
                {/* Category Table */}
                <CategoryInsightsTable rows={rows} />
              </div>
            </div>
          )
        })}
...
export default ClassroomInsightsReportSection
