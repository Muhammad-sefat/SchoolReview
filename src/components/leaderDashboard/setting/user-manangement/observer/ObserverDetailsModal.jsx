...
const ObserverDetailsModal = ({ isOpen, onClose, observer, onApprove, onDecline }) => {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-lg w-[95vw] rounded-3xl p-6 sm:p-8 space-y-6 bg-white border border-gray-200 shadow-2xl font-urbanist">
        <DialogHeader className="p-0 space-y-1 text-left pb-3 border-b border-gray-100">
          <DialogTitle asChild>
            <Title24 className="text-[#080808] font-semibold">Observer Details</Title24>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-6">
          <div className="space-y-1">
            ...
            <p className="text-[16px] font-normal text-[#080808]">{observer.name || "Courtney Henry"}</p>
          </div>

          <div className="space-y-1">
            <span className="text-[14px] font-normal text-[#5A5A5A]">Email</span>
            <p className="text-[16px] font-normal text-[#080808]">{observer.email || "courtneyhenry7@example.com"}</p>
          </div>

          <div className="space-y-1">
            <span className="text-[14px] font-normal text-[#5A5A5A]">Status</span>
            <p className="text-[16px] font-normal text-[#080808] capitalize">{observer.status || "Pending"}</p>
          </div>
        ...
        <div className="space-y-1 pt-1">
          ...
          <p className="text-[16px] font-normal text-[#080808] leading-relaxed">
            I would like to join your school as a Teacher Observer. Please review and approve my request.
          </p>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
          <button
            ...
            onClick={() => {
              onClose()
              if (onDecline) onDecline(observer)
            }}
          ...
          <button
            ...
            onClick={() => {
              onClose()
              if (onApprove) onApprove(observer)
            }}
...
export default ObserverDetailsModal
