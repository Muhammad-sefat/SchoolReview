...
import ObserverDetailsModal from "./ObserverDetailsModal"
import DeclineObserverModal from "./DeclineObserverModal"
import ApproveObserverModal from "./ApproveObserverModal"
...
const ObserverTableSection = () => {
  const [observers, setObservers] = useState(INITIAL_OBSERVERS)
  const [statusFilter, setStatusFilter] = useState("all")

  // Modals state
  const [viewingObserver, setViewingObserver] = useState(null)
  const [approvingObserver, setApprovingObserver] = useState(null)
  const [decliningObserver, setDecliningObserver] = useState(null)
  ...
  const pendingList = observers.filter((o) => o.status === "Pending")
  const approvedList = observers.filter((o) => o.status === "Verified" || o.status === "Approved")
  const declinedList = observers.filter((o) => o.status === "Declined")
  ...
  return (
    <div className="bg-white border border-gray-200/80 rounded-3xl p-6 sm:p-8 shadow-2xs space-y-6 font-urbanist">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        <Title24 className="text-[#080808]">Observers</Title24>
              ...
              >
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-2 text-[#1F1F21]">
                    ...
                    <button
                      type="button"
                      onClick={() => setViewingObserver(row)}
                    ...
                    {row.status === "Pending" ? (
                      <>
                        <button
                          type="button"
                          onClick={() => setApprovingObserver(row)}
                        ...
                        <button
                          type="button"
                          onClick={() => setDecliningObserver(row)}
                      ...
                      <button
                        type="button"
                        onClick={() => setDeletingObserver(row)}
      ...
      {/* Modals */}
      <ObserverDetailsModal
        isOpen={!!viewingObserver}
        onClose={() => setViewingObserver(null)}
        observer={viewingObserver}
        onApprove={(o) => setApprovingObserver(o)}
        onDecline={(o) => setDecliningObserver(o)}
      />

      <ApproveObserverModal
        isOpen={!!approvingObserver}
        onClose={() => setApprovingObserver(null)}
        observer={approvingObserver}
        onApproveConfirm={handleApproveConfirm}
      />

      <DeclineObserverModal
        isOpen={!!decliningObserver}
        onClose={() => setDecliningObserver(null)}
        observer={decliningObserver}
...
export default ObserverTableSection
