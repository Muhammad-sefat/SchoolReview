...
const DeleteTeacherModal = ({ isOpen, onClose, teacher, onDeleteConfirm }) => {
  if (!teacher) return null

  const handleDelete = () => {
    if (onDeleteConfirm) onDeleteConfirm(teacher.id)
    onClose()
  }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md w-[95vw] rounded-3xl p-6 sm:p-8 space-y-6 bg-white border border-gray-200 shadow-2xl font-urbanist">
        <DialogHeader className="p-0 space-y-1 text-left pb-3 border-b border-gray-100">
          <DialogTitle asChild>
            <Title24 className="text-[#080808] font-semibold">Delete Teacher?</Title24>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <p className="text-[18px] font-normal text-[#080808]">
            ...
            <span className="text-[#038AF9] font-medium">{teacher.name}</span>?
          </p>

          <p className="text-[16px] font-normal text-[#5A5A5A] leading-relaxed">
            This will permanently delete the teacher, remove their access to the dashboard, and delete all associated data, including reviews, reports, observations, and historical records. This action cannot be undone.
          </p>
        </div>
