...
const ApproveEvaluatorModal = ({ isOpen, onClose, evaluator, onApproveConfirm }) => {
  if (!evaluator) return null

  const handleApprove = () => {
    if (onApproveConfirm) onApproveConfirm(evaluator.id)
    onClose()
  }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md w-[95vw] rounded-3xl p-6 sm:p-8 space-y-6 bg-white border border-gray-200 shadow-2xl font-urbanist">
        <DialogHeader className="p-0 space-y-1 text-left pb-3 border-b border-gray-100">
          <DialogTitle asChild>
            <Title24 className="text-[#080808] font-semibold">Approve Evaluator Request?</Title24>
          </DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          <p className="text-[16px] font-normal text-[#5A5A5A] leading-relaxed">
            Are you sure you want to approve this evaluator? They will be granted access to your school and notified by email.
          </p>
        </div>
...
export default ApproveEvaluatorModal
