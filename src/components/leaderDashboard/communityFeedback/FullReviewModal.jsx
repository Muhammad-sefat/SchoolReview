...
const DetailStarRating = ({ rating = 4.5 }) => {
  ...
  for (let i = 1; i <= 5; i++) {
    if (i <= Math.floor(rating)) {
      stars.push(
        <svg key={i} xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" className="shrink-0">
...
// Full Category Dataset matching user screenshot
const ALL_CATEGORIES_REVIEWS = [
  {
    category: "Wellbeing",
    items: [
      {
    ...
    items: [
      {
        ...
        rating: 4,
        text: "Teachers explain topics clearly and are always willing to help when students need extra support.",
      },
      {
        title: "Individual Learning Support",
        rating: 3,
        text: "Most students receive the support they need, although more challenge for advanced learners would be welcome.",
      },
      {
        title: "Homework Load",
        rating: 3,
        text: "Homework is useful, but several subjects sometimes set work at the same time.",
      },
      {
        title: "Future Readiness",
        rating: 4,
        text: "The school encourages independence and prepares students well for the next stage of education.",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        rating: 4,
        text: "Teachers explain lessons clearly and help students understand new topics.",
      },
      {
      ...
      },
      {
        title: "Leadership",
        rating: 3,
        text: "School leadership is effective and creates a positive learning environment.",
      },
      {
        title: "Classroom Management",
        rating: 3,
        text: "Classes are well managed, with respectful behaviour and effective teaching practices.",
      },
    ],
  },
]
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose && onClose()}>
        <DialogContent className="max-w-[960px] w-[95%] sm:w-full rounded-[32px] bg-white p-5 sm:p-6 border border-gray-100 shadow-2xl font-urbanist max-h-[92vh] overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-gray-300/80 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-gray-400">
          {/* Modal Header Title */}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-4 text-sm sm:text-base">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 text-base font-normal ml-2">
                  <img src={checkImage} alt="check" className="w-5 h-5 object-contain" />
                  <span className="text-textBlack text-base font-medium">Recommended</span>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-start bg-white">
                    <button
                      ...
                      className="text-[#038AF9] hover:underline text-[14px] sm:text-[15px] font-medium inline-flex items-center gap-2 cursor-pointer transition-colors"
                    >
                      {isExpanded ? (
                        <>
                        ...
                        <>
                          <Maximize2 className="w-4 h-4 text-[#038AF9]" />
                      ...
                      <div className="space-y-6 animate-fadeIn transition-all duration-500">
                        {ALL_CATEGORIES_REVIEWS.map((catGroup, groupIdx) => (
                          <div key={groupIdx} className="space-y-4">
                            ...
                            {/* Review Items inside Category */}
                            <div className="space-y-4">
                              {catGroup.items.map((item, itemIdx) => (
                                <div key={itemIdx} className="space-y-1.5 font-urbanist">
                                  <h4 className="text-[16px] font-semibold text-[#080808]">
                                    {item.title}
                                  </h4>
                                  <div className="flex items-center gap-3 flex-wrap pt-0.5">
                                    <DetailStarRating rating={item.rating} />
                                    {item.text && (
                                      <p className="text-[15px] sm:text-[16px] font-normal text-textPrimary leading-snug">
                                        {item.text}
                                      </p>
                                    )}
                                  </div>
                            ...
                            {/* Dotted Divider between Category Blocks (except last) */}
                            {groupIdx < ALL_CATEGORIES_REVIEWS.length - 1 && (
