...
const SuggestionModal = ({
  ...
  suggestedText = "I find the lesson pace too fast and sometimes struggle to understand what is being taught. More time to process the material would help me keep up.",
}) => {
  ...
  const handleContinueClick = () => {
    let finalSelection = suggestedText
    if (activeTab === "edit") {
      finalSelection = editedText
    }

    if (onUseSuggestion) onUseSuggestion(finalSelection)
    if (onContinue) onContinue(finalSelection)
    handleClose()
  }

  return (
    <Dialog open={!!isModalOpen} onOpenChange={(val) => { if (!val) handleClose() }}>
      <DialogContent className="max-w-[540px] bg-white rounded-[24px] shadow-2xl p-6 sm:p-7 space-y-4 font-urbanist border border-[#EAEAEA]">
        {/* Header */}
        <div className="flex items-center justify-between gap-3 pr-6">
          ...
          {activeTab === "why" && (
            <a
              ...
              className="text-[#038AF9] hover:underline text-xs sm:text-sm font-medium"
            >
              View guidelines
            </a>
          )}
          ...
          <div className="pt-1 space-y-3">
            ...
            <div className="border border-[#FB8C00] rounded-xl p-4 bg-[#FFF9F0] flex gap-3 items-start">
              ...
              <span className="text-xs sm:text-sm font-normal text-[#1F1F21] leading-relaxed">
                Personal attacks or unconstructive phrasing can feel unfair and reduce the impact of your feedback. Focusing on specific experiences makes your review more credible and actionable.
              </span>
            </div>
          </div>
        )}
